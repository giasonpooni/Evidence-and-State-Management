...
export const BLOCKERS: Record<'notation' | 'release' | 'fmcsaRail' | 'railDisabled', Blocker> = {
  ...
  release: { what: 'The admission ruling now exists and is installed at the write boundary — checks that refuse by default, an authority that is never the method itself, an ancestry ledger the release does not carry, and a response pipeline that refuses to serve a row which never crossed it. The ruling is called on the statutory harvester path and by the admit CLI; no candidate build from this rail has been put through it, so every build stays UNADMITTED and the release history is the committed corpus.', owner: 'Admission contract (docs/GOVERNANCE_KERNEL.md: one authoritative route for evaluation, identity, version history and release eligibility)', remedy: 'A candidate build routed through the ruling with a named authority; then an internal release separate from demonstration fixtures.' },
