...
The reference behavior is defined by Bench `src/corpus-build-diff-workflow.js` and `test/corpus-build-diff-workflow.test.js` at `c6d693613478f32e0b0d7dafe918d8e51274ffcc`: exact immutable references, explicit same-identity digest differences, and no inferred semantic change. These references are not runtime dependencies.
