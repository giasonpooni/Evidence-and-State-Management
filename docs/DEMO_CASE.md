...
`src/fixtures/caravan/release.ts` holds corpus `caravan.specialty-cargo`: three releases (`REL-CAR-2026.08.11`, `REL-CAR-2026.08.25`, `REL-CAR-2026.09.01`, the last current), nineteen records about seven lots and the samples drawn from them, two retractions, and the rights schedule of seven sources. Records carry uncertainty bounds, validity bounds, both clocks, provenance with content hashes, evidence class and `notation://` identities. The sources are operational (weighbridges, custody systems, carrier documents, a claimant's yard log, a sponsor's deal terms) and scientific (a laboratory information system); geospatial and remote-sensing material is not represented, and the product page says so. Release digests are sha256 over the canonical record set and are stamped and drift-tested like every other digest.

What the corpus demonstrates: an as-of answer that changes with the knowledge time (lot 5B-221 quantity is 40.000 t estimated before 2026-08-25 14:00 UTC and 40.120 t measured, bounds [40.08, 40.16], after the correction RET-0001); a fact that reaches a lot only through an identity-link record (lot 5B-221 moisture via sample S-4402) and a typed refusal where no link exists (lot 7C-104 moisture: sample S-4418 is never linked, which is exactly what the workbench ruling refused on); a withdrawal (RET-0002, certificate NIS-4390) that an earlier release still shows as current and that names the ruling it touched (RUL-3F440-r1); and a source whose rights schedule forbids customer delivery, whose one record never leaves the corpus.

## Which workbench fixtures are used
