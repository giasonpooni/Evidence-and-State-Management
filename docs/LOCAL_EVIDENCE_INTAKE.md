...
| `src/corpus-workflow.js` | Deterministic membership and a recomputable build root are separate from byte capture. The local candidate builder now assembles explicit normalized-candidate references into an unadmitted manifest; intake itself performs no build. |
